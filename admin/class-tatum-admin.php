...
class Tatum_Admin
{
    ...
    private $chains = ['ETH' => 'ETH', 'CELO' => 'CELO', 'BSC' => 'BSC'];
        ...
        <tr>
            ...
            <td>
                <select id="<?php echo $name; ?>"
                        name="<?php echo $name; ?>" <?php echo $readonly ? 'disabled' : ''; ?>>
...
<!--            <span id="api_key_status" style="display: none">--><?php //echo $status ?><!--</span>-->
    ...
    public function format_api_key_status_long(WP_Post $post) {
        $status = get_post_meta($post->ID, 'status', true);
    ...
    public function woocommerce_add_transaction_data_column($product, $item, $item_id) {
        ...
        <td class="line_transfer_hash">
            <?php echo get_post_meta($product->get_id(), 'tatum_transfer_hash', true); ?>
        </td>
        <?php
    }
